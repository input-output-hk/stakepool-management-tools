...
const ConfigureNode = () => (
  <div className="contentModal">
    ...
    <p>
      ...
      <code className="inline-code">config.yaml</code>{' '}
      <a
      ...
      The <code className="inline-code">config.yaml</code> file has the list of
      trusted peers. From this list the node is going to download its copy of
      the blockchain.
    </p>
    ...
    <ol>
      <li>
        Execute the <code className="inline-code">node stats</code> command from
        above multiple times and check that the value of the
        <code className="inline-code">lastBlockHash</code>lastBlockHash field is
        updated (with the actual blockchain values, there should bea new block
        created every 2-5 minutes);
      </li>
      <li>
        Compare the value of the{' '}
        <code className="inline-code">lastBlockTime</code> field (that is in
        UTC) with the local time of the node. If the difference if more than 10
        minutes, the node might not be synced even the node received blocks in
        the past <code className="inline-code">(blockRecvCnt &gt; 0);</code>
      </li>
    </ol>
  </div>
);
